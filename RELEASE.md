...
**시크릿은 실행 시점에 읽힙니다.** 이미 끝난 배포에 나중에 시크릿을 넣어도 그 배포는 다시
안내하지 않습니다. 다음 푸시부터 적용됩니다.

### 3. 보내지 않고 먼저 확인하기

배포를 기다리지 않고 지금 모양을 보고 싶으면 로컬에서 직접 보낼 수 있습니다.

```bash
node scripts/announce-update.mjs 0.0.0-test
```

설명 파일을 안 주면 지난 태그 이후의 커밋으로 직접 만듭니다.
웹훅 URL 은 환경변수로 넣습니다 — PowerShell 이면 `$env:DISCORD_UPDATE_WEBHOOK_URL = "..."`.
