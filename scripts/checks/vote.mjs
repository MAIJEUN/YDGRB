...
assert(
  "옵션은 참가인원·기간·제목",
  options.map((o) => o.name).join(",") === "참가인원,기간,제목",
...
console.log("\n=== 9. 표는 채널에 안 뜬다 ===");
{
  ...
  assert("누른 사람에게만 답함", source.includes("판 화면도 건드리지 않는다"));
  assert("  └ 화면을 다시 그리지 않음", !source.includes("refreshPanel(interaction.client, running"));
