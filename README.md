...
| 유저는 항상 멘션 | 내용·변동에서 사람을 가리킬 때는 `<@id>`. 이름을 글자로 적지 않습니다 (footer 만 예외) |
...
역할 이름은 멘션이 아니라 `` `@이름` `` 으로 적습니다 — 역할 멘션은 알림을 보내고,
`@everyone` 멘션은 서버 전체에 알림이 갑니다. 유저는 반대로 항상 멘션으로 적습니다.
`@everyone` 과 `@here` 는 이름 자체에 `@` 가 들어 있어 덧붙이지 않습니다.
