...
| 오류도 같은 컨테이너로 | 실패(빨강) + `원인` 필드에 짧은 오류 메시지 ([errors.ts](src/errors.ts)) |
...
> 올라간 임베드 메시지(예: 처리 대기 중인 소원)는 컨테이너로 갈아 끼울 수 없어서,
> 수락/거절 시 버튼만 바꾸고 결과를 답글로 남기는 옛 경로를 그대로 탑니다
...
역할의 **메시지 보내기** 권한을 켜고 끕니다. 지금 상태를 읽어서 뒤집는 토글이라 같은 명령으로 되돌립니다.
...
**별명 관리**가 아니라 **역할 관리(Manage Roles)** 권한이 필요합니다. 권한 없는 사람에게는 명령이 보이지 않습니다.

### 바꿀 수 없는 역할

| 대상 | 이유 |
|------|------|
| 봇·연동이 관리하는 역할 | 디스코드가 누구에게도 권한 변경을 허용하지 않습니다 |
| 봇보다 높은 역할 | 서버 설정 > 역할 에서 봇 역할을 위로 올려야 합니다 |
| 자신보다 높거나 같은 역할 | 서버 소유자는 예외입니다 |

`@everyone` 은 항상 가장 아래라 봇에게 역할 관리 권한만 있으면 바꿀 수 있습니다.

> **채널에 따로 지정된 권한이 있으면 그쪽이 우선합니다.** 역할 권한을 꺼도 특정 채널에서
> 그 역할에 「메시지 보내기」를 허용해 두었다면 그 채널에서는 계속 쓸 수 있습니다.
>
> `@everyone` 을 끄면 봇도 `@everyone` 으로 채팅 권한을 얻고 있었을 경우 일반 메시지를 못 보냅니다.
> (슬래시 커맨드 응답은 그와 무관하게 나가므로 다시 켜는 데는 지장이 없습니다)

역할 이름은 멘션이 아니라 `` `@이름` `` 으로 적습니다 — 멘션을 쓰면 알림이 갈 수 있기 때문입니다.
...
관리자: 전달된 메시지의 [수락] / [거절]
  ├ 원본 메시지가 결과 색(초록/빨강)으로 다시 그려짐 — 이미지·첨부는 그대로
  ├ 버튼은 비활성 「수락됨 / 거절됨」 으로 바뀜
