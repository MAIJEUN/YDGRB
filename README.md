...
│  ├─ targets.ts             유저 칸 한 개에서 멘션·ID·이름 풀어내기
...
```
/별명 뚜따이 별명:요땅그래
/별명 뚜따이 별명:요땅그래 유저:마이즌, 아무개 기간:1일 4시간
/별명 바사삭
/별명 바사삭 유저:@마이즌
```
...
|------|:------:|:------:|------|
| `별명` | ✅ 필수 | — | 바꿀 별명 (32자까지) |
| `유저` | 선택 | 선택 | **지목한 사람만** 바꿉니다. 비우면 서버 전원 |
| `기간` | 선택 | — | 지나면 **자동으로 바사삭**됩니다. 비우면 직접 풀 때까지 유지 |

### 유저 칸 하나에 여러 명

슬래시 커맨드에는 여러 명을 고르는 옵션 자체가 없어서 **문자열 한 칸**으로 받고
[targets.ts](src/nickname/targets.ts) 에서 풉니다. 섞어 써도 됩니다.

| 적는 법 | 예 |
|---------|-----|
| **자동완성** | 이름을 치면 후보가 뜨고, 고르면 쉼표로 이어 붙습니다 |
| 멘션 | `@마이즌 @아무개` (붙여넣기) |
| ID | `750400239625568306 123456789012345678` |
| 이름 | `마이즌, 아무개` — **쉼표로 구분** (이름에 공백이 있을 수 있어서) |

- 사용자명 · 전역 이름 · 서버 별명 어느 쪽으로 적어도 찾습니다 (대소문자 무관, 부분 일치)
- 같은 사람을 두 번 적어도 한 번만 처리합니다
- **같은 이름이 여럿이거나 못 찾으면 실행하지 않고** 어떤 값이 문제인지 알려 줍니다.
  의도한 인원보다 적게 바꾸는 게 더 나쁘기 때문입니다 — 이때는 멘션이나 ID 로 적으면 확실합니다
- 자동완성 후보 값은 디스코드 제한(100자)이 있어 대여섯 명쯤에서 멈춥니다. 그 이상은 멘션을 붙여넣으세요

지목한 사람은 한 명씩 REST 로 가져옵니다 — 멤버 목록 요청 제한과 무관하고 즉시 끝납니다.
