...
```
/별명 뚜따이 별명:요땅그래
/별명 뚜따이 별명:요땅그래 유저:@마이즌 유저2:@아무개 기간:1일 4시간
/별명 바사삭
/별명 바사삭 유저:@마이즌
```
...
|------|:------:|:------:|------|
| `별명` | ✅ 필수 | — | 바꿀 별명 (32자까지) |
| `유저` ~ `유저5` | 선택 | 선택 | **지목한 사람만** 바꿉니다. 전부 비우면 서버 전원 |
| `기간` | 선택 | — | 지나면 **자동으로 바사삭**됩니다. 비우면 직접 풀 때까지 유지 |

> 슬래시 커맨드에는 여러 명을 한 번에 고르는 옵션이 없어서 **유저 칸을 5개** 두었습니다.
> 문자열로 멘션을 받는 방법도 있지만, 정식 유저 선택기라야 id 가 정확히 들어옵니다.
> 같은 사람을 두 칸에 넣어도 한 번만 처리합니다. 더 필요하면 `USER_SLOTS` 에 칸을 늘리면 됩니다.
