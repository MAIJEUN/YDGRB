...
```
/별명 뚜따이     ← 모달에서 별명과 기간을 받습니다
/별명 바사삭     ← 즉시 초기화
```

| 하위 명령 | 동작 |
|-----------|------|
| 뚜따이 | 모두의 별명을 정한 값으로 바꿉니다. 기간을 적으면 지나면 **자동으로 바사삭**됩니다 (비우면 직접 풀 때까지 유지) |
| 바사삭 | 모두의 별명을 **지웁니다**. 예전 별명을 저장해 되돌리는 게 아니라, 별명을 없애면 사용자명으로 돌아가는 디스코드 동작을 그대로 씁니다 |
...
**1. 인텐트** — [src/client.ts](src/client.ts) 에서 관리합니다. 지금은 `Guilds` 와 `GuildMembers` 를 씁니다.
