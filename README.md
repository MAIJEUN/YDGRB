...
|------|------|------|
| `별명` | ✅ | 바꿀 별명 (32자까지) |
| `유저` | | **이 사람만** 바꿉니다. 비우면 서버 전원 |
| `기간` | | 지나면 **자동으로 바사삭**됩니다. 비우면 직접 풀 때까지 유지 |

한 명만 지정하면 그 사람만 REST 로 가져와 바꿉니다 — 멤버 목록 요청 제한과 무관하고 즉시 끝납니다.
...
반대로 **전체 바사삭은** 모두의 별명을 지우므로 개별 예약까지 전부 정리합니다.
