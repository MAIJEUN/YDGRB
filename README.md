...
- **[채팅뻥](#채팅뻥)** — 역할의 채팅을 모든 채널에서 한 번에 막고 풀기
...
역할의 채팅을 서버의 **모든 채널에서** 막거나 풉니다. 지금 상태를 읽어서 뒤집는 토글이라
같은 명령으로 되돌립니다.

```
/채팅뻥              ← 비우면 @everyone (서버 전체)
/채팅뻥 역할:@손님
```
...
채널 덮어쓰기의 **차단**은 역할 권한을 이깁니다. 그래서 모든 채널에 그 역할의
「메시지 보내기」 차단을 걸었다 푸는 방식으로 동작합니다.

| | 하는 일 |
|---|---|
| 끄기 | 모든 채널의 덮어쓰기에 `SendMessages: 차단` 을 넣습니다 |
| 켜기 | 그 항목을 **빼서** 원래대로(상속) 되돌립니다. 명시적 허용으로 바꾸지 않습니다 |

- **스레드는 건드리지 않습니다** — 부모 채널의 권한을 물려받습니다
- **카테고리도 포함합니다** — 나중에 만드는 채널이 같은 상태로 시작합니다
- 같은 덮어쓰기의 다른 항목은 그대로 둡니다 (`permissionOverwrites.edit` 은 지정한 것만 손댑니다)
- 채널 수만큼 요청이 나가므로 진행률을 실시간으로 보여 주고, 끝나면 전체·완료·실패를 셉니다
...
봇이 볼 수 없거나 봇에게 **역할 관리** 권한이 없는 채널은 건너뛰고 실패로 셉니다.

> `@everyone` 을 끄면 봇도 일반 메시지를 못 보내게 될 수 있습니다.
