...
export interface GameDefinition {
  ...
  /**
   * 판 화면에 게임이 **덧붙이는 칸.** 골격이 적는 칸(인원 · 마감 · 종료) 뒤에 붙는다.
   *
   * 내용 한 덩어리로는 모자란 게임이 쓴다 — 찍기대작전의 「참가한 사람」처럼, 본문과
   * 나란히 두면 무엇이 게임이고 무엇이 명단인지 흐려지는 것.
   *
   * 화면을 다시 그릴 때마다 불린다. 끝난 판에는 부르지 않는다 (그 자리는 결과가 쓴다).
   */
  fields?(session: GameSession): readonly ResponseField[];
