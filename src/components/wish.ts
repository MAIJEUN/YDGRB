  ...
  if (!result.ok) {
    await replaceViewAndAnnounce(
      ...
      noticeView({
        ...
        fields: [{ name: "현재 보유", value: formatBalance(result.before), inline: true }],
  ...
  if (balance.tickets < 1) {
    await replaceView(
      ...
      noticeView({
        ...
        fields: [{ name: "현재 보유", value: formatBalance(balance), inline: true }],
        user: interaction.user,
        panel: PANEL.user,
      }),
    );
  ...
  if (!spent.ok) {
    await replaceView(
      ...
      noticeView({
        ...
        fields: [{ name: "현재 보유", value: formatBalance(spent.before), inline: true }],
        user: interaction.user,
        panel: PANEL.user,
      }),
    );
...
function wishMessage(
  ...
  return {
    ...
    title: "새 소원",
    description: content,
    fields: [{ name: "신청자", value: `<@${requester.id}>`, inline: true }],
    ...
    await target.send(
      channelMessage({
        ...
        fields: [
          ...
          { name: "원본", value: `[소원 보러 가기](${messageUrl})`, inline: true },
          ...(accepted ? [] : [{ name: "안내", value: "소원권 1장을 돌려드렸어요.", inline: true }]),
        ],
        balance: refundText,
        user: target,
        ephemeral: false,
  ...
  if (!drained.ok) {
    await replaceViewAndAnnounce(
      ...
      noticeView({
        ...
        fields: [{ name: "현재 보유", value: formatBalance(drained.before), inline: true }],
        user: interaction.user,
        panel: PANEL.admin,
      }),
    );
