 ...
 *
 *  10. **봇은 말끝을 맺지 않는다.** 화면에 나가는 문장은 [tone.ts](tone.ts) 의
 *      `speak()` 를 거친다 — 「끝났습니다」가 아니라 「끝났ㅅ-」이다.
 *
 *      새 문장을 쓸 때 손으로 잘라 두지 말 것. 표는 한 곳에만 있고, 잘린 꼴을 소스에
 *      박아 두면 말투를 바꾸는 날 그 문장만 남는다. 잘라 놓은 문장은 검색도 안 된다.
 *
 *      **유저가 적은 글에는 쓰지 않는다** — 퀴즈의 문제·정답, 별명, 사유, 초성 본보기.
 *      그건 봇의 말이 아니라 데이터라, 말투를 씌우면 뜻이 바뀌거나 사라진다.
 *      로그와 개발자 오류도 사람이 읽는 화면이 아니니 그대로 둔다.
 *
 *  11. **한번 나간 메시지의 색은 나중에 바꾸지 않는다.** 색은 그 메시지가 **무엇인지**를
 *      말하는 것이지, 그 뒤에 무슨 일이 일어났는지를 말하는 것이 아니다.
 *
 *      「누가 소원을 빌었다」 는 수락되든 거절되든 그대로다. 「판이 돌고 있다」 도 그 판이
 *      끝난 뒤에 거짓이 되지 않는다 — 그때 돌고 있었던 것은 사실이다. 그래서 빈 소원도
 *      미니게임 판도 처음 색(노랑)으로 남는다.
 *
 *      무슨 일이 일어났는지는 **다음 메시지**가 말한다 (규칙 7의 답장). 원본에서 바꾸는
 *      것은 색이 아니라 **버튼**이다 — 「수락」이 눌리지 않는 「수락됨」이 되고, 끝난 판에는
 *      버튼이 사라진다. 그래야 위를 훑는 사람은 무엇이 오갔는지를 보고, 방금 온 사람은
 *      맨 아래 답장 하나로 결말을 안다.
 *
 *      **예외는 아직 아무도 못 본 화면뿐이다** — 패널처럼 누른 사람에게만 보이고 그 자리에서
 *      갈아 끼우는 것. 그건 같은 화면의 다음 장이지 지나간 기록이 아니다.
 */
